import { PHASE34_TECHNIQUES } from './techniquesPhase34.js';

export const CATEGORIES = [
  { id: 'multiply', icon: '✕', en: 'Multiply', hi: 'गुणा' },
  { id: 'divide', icon: '÷', en: 'Divide', hi: 'भाग' },
  { id: 'squares', icon: '²', en: 'Squares', hi: 'वर्ग' },
  { id: 'cubes', icon: '³', en: 'Cubes', hi: 'घन' },
  { id: 'roots', icon: '√', en: 'Roots', hi: 'मूल' },
  { id: 'mental', icon: '+', en: 'Mental Add', hi: 'मानसिक जोड़' },
  { id: 'percent', icon: '%', en: 'Percent', hi: 'प्रतिशत' },
  { id: 'fractions', icon: '½', en: 'Fractions', hi: 'भिन्न' },
  { id: 'numbers', icon: '🔢', en: 'HCF/LCM', hi: 'HCF/LCM' },
  { id: 'commercial', icon: '₹', en: 'Money Math', hi: 'पैसे' },
  { id: 'algebra', icon: '𝑥', en: 'Algebra', hi: 'बीजगणित' },
  { id: 'mensuration', icon: 'π', en: 'Mensuration', hi: 'क्षेत्रमिति' },
  { id: 'trig', icon: '∠', en: 'Trig', hi: 'त्रिकोणमिति' },
  { id: 'verify', icon: '✓', en: 'Check', hi: 'जाँच' },
];

export const PRO_TYPES = {
  speed: { icon: '⚡', en: 'Faster', hi: 'तेज़' },
  steps: { icon: '📉', en: 'Fewer steps', hi: 'कम कदम' },
  mental: { icon: '🧠', en: 'In your head', hi: 'दिमाग में' },
  pattern: { icon: '🔁', en: 'Works for many', hi: 'कई पर काम करता है' },
  exam: { icon: '📝', en: 'Exam-friendly', hi: 'परीक्षा के लिए' },
  verify: { icon: '✓', en: 'Easy to check', hi: 'आसान जाँच' },
  confidence: { icon: '💪', en: 'Builds confidence', hi: 'आत्मविश्वास बढ़ाता है' },
};

export const TECHNIQUES = [
  {
    id: 'multiply-11',
    category: 'multiply',
    difficulty: 1,
    icon: '11',
    en: 'Multiply by 11',
    hi: '11 से गुणा',
    benefitChip: { en: '2× faster', hi: '2 गुना तेज़' },
    duration: '3 min',
    whenToUse: {
      en: 'Best for: homework, mental tests, quick estimates',
      hi: 'सबसे अच्छा: होमवर्क, मानसिक गणित, जल्दी अनुमान',
    },
    whenNotToUse: {
      en: 'For very large numbers, long multiplication is fine too.',
      hi: 'बहुत बड़े अंकों के लिए सामान्य गुणा भी ठीक है।',
    },
    whyVedic: [
      { type: 'steps', en: 'Fewer steps than column multiply', hi: 'कॉलम गुणा से कम कदम' },
      { type: 'mental', en: 'Do it entirely in your head', hi: 'पूरा दिमाग में करें' },
      { type: 'pattern', en: 'Same trick for 11, 22, 33…', hi: '11, 22, 33… पर एक ही तरीका' },
      { type: 'verify', en: 'Check with digit sum in 1 second', hi: 'अंक जोड़ से 1 सेकंड में जाँच' },
    ],
    regular: {
      steps: [
        { en: 'Write 47 × 11 in columns', hi: '47 × 11 कॉलम में लिखें' },
        { en: 'Multiply 7 × 1 = 7', hi: '7 × 1 = 7 गुणा करें' },
        { en: 'Multiply 4 × 1, carry over', hi: '4 × 1 गुणा, कैरी लें' },
        { en: 'Add partial products → 517', hi: 'आंशिक जोड़ें → 517' },
      ],
      timeEstimate: 22,
    },
    vedic: {
      steps: [
        { en: 'Split 47: 4 | 7', hi: '47 विभाजित करें: 4 | 7', proTip: { en: 'Keep the outer digits separate.', hi: 'बाहरी अंक अलग रखें।' } },
        { en: 'Add middle: 4 + 7 = 11', hi: 'बीच जोड़ें: 4 + 7 = 11', proTip: { en: 'Add the two digits — this is the middle part.', hi: 'दो अंक जोड़ें — यह बीच का भाग है।' } },
        { en: 'Write 1 in middle, carry 1', hi: 'बीच में 1 लिखें, 1 कैरी करें', proTip: { en: 'When the sum is 11 or more, carry to the left.', hi: 'योग 11 या अधिक हो तो बाएँ कैरी करें।' } },
        { en: 'Add carry: 4 + 1 = 5', hi: 'कैरी जोड़ें: 4 + 1 = 5', proTip: { en: 'Add the carry to the left digit.', hi: 'कैरी को बाएँ अंक में जोड़ें।' } },
        { en: 'Answer: 5 | 1 | 7 → 517', hi: 'जवाब: 5 | 1 | 7 → 517', proTip: { en: 'Read left, middle, right — done!', hi: 'बाएँ, बीच, दाएँ पढ़ें — हो गया!' } },
      ],
      pros: ['steps', 'mental', 'speed'],
      timeEstimate: 8,
    },
    practice: [
      { q: '23 × 11', a: 253, hint: { en: '2+3=5 in the middle', hi: 'बीच में 2+3=5' } },
      { q: '45 × 11', a: 495, hint: { en: '4+5=9 in the middle', hi: 'बीच में 4+5=9' } },
      { q: '72 × 11', a: 792, hint: { en: '7+2=9 in the middle', hi: 'बीच में 7+2=9' } },
      { q: '56 × 11', a: 616, hint: { en: '5+6=11 — write 1 in middle, carry 1 to left', hi: '5+6=11 — बीच में 1, बाएँ 1 कैरी' } },
      { q: '38 × 11', a: 418, hint: { en: '3+8=11 — write 1 in middle, carry 1 to left', hi: '3+8=11 — बीच में 1, बाएँ 1 कैरी' } },
    ],
    masteredText: { en: 'You can now multiply by 11 in your head!', hi: 'अब आप 11 से गुणा दिमाग में कर सकते हैं!' },
  },
  {
    id: 'multiply-25',
    category: 'multiply',
    difficulty: 1,
    icon: '25',
    en: 'Multiply by 25',
    hi: '25 से गुणा',
    benefitChip: { en: 'No long division', hi: 'लंबा भाग नहीं' },
    duration: '3 min',
    whenToUse: {
      en: 'Best for: money problems, quick mental math',
      hi: 'सबसे अच्छा: पैसे के सवाल, जल्दी मानसिक गणित',
    },
    whenNotToUse: {
      en: 'Numbers not divisible by 4 need an extra step.',
      hi: '4 से न भाज्य अंकों में एक अतिरिक्त कदम लगेगा।',
    },
    whyVedic: [
      { type: 'speed', en: 'Divide by 4, then ×100 — super fast', hi: '4 से भाग, फिर ×100 — बहुत तेज़' },
      { type: 'mental', en: 'Think in quarters (25¢ = quarter dollar)', hi: 'चौथाई में सोचें (25 पैसे = चौथाई)' },
      { type: 'pattern', en: 'Works for 25, 250, 2500…', hi: '25, 250, 2500… पर काम करता है' },
      { type: 'exam', en: 'Common in word problems', hi: 'शब्दों वाले सवालों में आम' },
    ],
    regular: {
      steps: [
        { en: 'Set up 48 × 25', hi: '48 × 25 सेट करें' },
        { en: 'Multiply 48 × 5 = 240', hi: '48 × 5 = 240 गुणा करें' },
        { en: 'Multiply 240 × 5 = 1200', hi: '240 × 5 = 1200 गुणा करें' },
        { en: 'Or use long multiplication', hi: 'या लंबी गुणा करें' },
      ],
      timeEstimate: 25,
    },
    vedic: {
      steps: [
        { en: '48 ÷ 4 = 12', hi: '48 ÷ 4 = 12', proTip: { en: '25 is 100÷4 — so divide first!', hi: '25 = 100÷4 — पहले भाग दें!' } },
        { en: '12 × 100 = 1200', hi: '12 × 100 = 1200', proTip: { en: 'Just add two zeros — instant!', hi: 'बस दो शून्य जोड़ें — तुरंत!' } },
      ],
      pros: ['speed', 'steps', 'mental'],
      timeEstimate: 6,
    },
    practice: [
      { q: '36 × 25', a: 900, hint: { en: '36÷4=9, then ×100', hi: '36÷4=9, फिर ×100' } },
      { q: '52 × 25', a: 1300, hint: { en: '52÷4=13, then ×100', hi: '52÷4=13, फिर ×100' } },
      { q: '64 × 25', a: 1600, hint: { en: '64÷4=16, then ×100', hi: '64÷4=16, फिर ×100' } },
      { q: '88 × 25', a: 2200, hint: { en: '88÷4=22, then ×100', hi: '88÷4=22, फिर ×100' } },
      { q: '44 × 25', a: 1100, hint: { en: '44÷4=11, then ×100', hi: '44÷4=11, फिर ×100' } },
    ],
    masteredText: { en: 'You can crush ×25 problems in seconds!', hi: 'अब आप ×25 के सवाल सेकंडों में हल कर सकते हैं!' },
  },
  {
    id: 'squares-5',
    category: 'squares',
    difficulty: 1,
    icon: '5²',
    en: 'Squares ending in 5',
    hi: '5 पर समाप्त वर्ग',
    benefitChip: { en: 'Instant squares', hi: 'तुरंत वर्ग' },
    duration: '4 min',
    whenToUse: {
      en: 'Best for: squares like 25², 35², 95²',
      hi: 'सबसे अच्छा: 25², 35², 95² जैसे वर्ग',
    },
    whenNotToUse: {
      en: 'Only works when the number ends in 5.',
      hi: 'केवल 5 पर समाप्त संख्याओं पर काम करता है।',
    },
    whyVedic: [
      { type: 'speed', en: 'Answer in 4 quick steps', hi: '4 त्वरित कदमों में जवाब' },
      { type: 'mental', en: 'No multiplication tables needed', hi: 'गुणा तालिका की जरूरत नहीं' },
      { type: 'pattern', en: 'Same rule for any …5 number', hi: 'किसी भी …5 संख्या पर एक नियम' },
      { type: 'confidence', en: 'Looks like magic to friends!', hi: 'दोस्तों को जादू जैसा लगेगा!' },
    ],
    regular: {
      steps: [
        { en: '35 × 35 — set up long multiplication', hi: '35 × 35 — लंबी गुणा सेट करें' },
        { en: '5×5=25, 5×3=15, carry…', hi: '5×5=25, 5×3=15, कैरी…' },
        { en: '3×5=15, 3×3=9, add all', hi: '3×5=15, 3×3=9, सब जोड़ें' },
        { en: 'Answer: 1225', hi: 'जवाब: 1225' },
      ],
      timeEstimate: 30,
    },
    vedic: {
      steps: [
        { en: 'Take 3 (digit before 5 in 35)', hi: '3 लें (35 में 5 से पहले का अंक)', proTip: { en: 'Ignore the 5 for now — focus on the digit before it!', hi: 'अभी 5 को छोड़ें — उससे पहले के अंक पर ध्यान दें!' } },
        { en: 'Next number: 3 + 1 = 4', hi: 'अगली संख्या: 3 + 1 = 4', proTip: { en: 'Always add 1 to the first digit.', hi: 'पहले अंक में हमेशा 1 जोड़ें।' } },
        { en: 'Multiply: 3 × 4 = 12', hi: 'गुणा करें: 3 × 4 = 12', proTip: { en: 'Multiply the two numbers from steps 1 and 2.', hi: 'कदम 1 और 2 की दोनों संख्याओं को गुणा करें।' } },
        { en: 'Append 25 → 1225', hi: '25 जोड़ें → 1225', proTip: { en: 'Squares ending in 5 always end with 25!', hi: '5 पर समाप्त वर्ग हमेशा 25 से समाप्त होते हैं!' } },
      ],
      pros: ['speed', 'mental', 'pattern'],
      timeEstimate: 8,
    },
    practice: [
      { q: '25²', a: 625, hint: { en: '2×3=6, append 25', hi: '2×3=6, 25 जोड़ें' } },
      { q: '45²', a: 2025, hint: { en: '4×5=20, append 25', hi: '4×5=20, 25 जोड़ें' } },
      { q: '65²', a: 4225, hint: { en: '6×7=42, append 25', hi: '6×7=42, 25 जोड़ें' } },
      { q: '85²', a: 7225, hint: { en: '8×9=72, append 25', hi: '8×9=72, 25 जोड़ें' } },
      { q: '15²', a: 225, hint: { en: '1×2=2, append 25', hi: '1×2=2, 25 जोड़ें' } },
    ],
    masteredText: { en: 'You can square any …5 number instantly!', hi: 'अब आप किसी भी …5 संख्या का वर्ग तुरंत निकाल सकते हैं!' },
  },
  {
    id: 'near-base',
    category: 'multiply',
    difficulty: 2,
    icon: '≈',
    en: 'Multiply near 100',
    hi: '100 के पास गुणा',
    benefitChip: { en: 'Big numbers, small work', hi: 'बड़े अंक, कम मेहनत' },
    duration: '5 min',
    whenToUse: {
      en: 'Best for: 98×97, 96×94, numbers close to 100',
      hi: 'सबसे अच्छा: 98×97, 96×94, 100 के पास के अंक',
    },
    whenNotToUse: {
      en: 'When numbers are far from 100, use regular method.',
      hi: 'जब अंक 100 से दूर हों, सामान्य तरीका अपनाएँ।',
    },
    whyVedic: [
      { type: 'speed', en: 'Turn hard multiply into easy subtract', hi: 'कठिन गुणा को आसान घटाव में बदलें' },
      { type: 'mental', en: 'Work with small differences only', hi: 'सिर्फ छोटे अंतर से काम करें' },
      { type: 'pattern', en: 'Base method works for 10, 100, 1000…', hi: 'आधार विधि 10, 100, 1000… पर काम करती है' },
      { type: 'exam', en: 'Saves time in competitive tests', hi: 'प्रतियोगी परीक्षाओं में समय बचाता है' },
    ],
    regular: {
      steps: [
        { en: '98 × 97 — long multiplication', hi: '98 × 97 — लंबी गुणा' },
        { en: '7×8=56, carries everywhere', hi: '7×8=56, हर जगह कैरी' },
        { en: 'Add partial products', hi: 'आंशिक जोड़ें' },
        { en: 'Answer: 9506', hi: 'जवाब: 9506' },
      ],
      timeEstimate: 35,
    },
    vedic: {
      steps: [
        { en: 'Find gaps from 100: 98 → -2, 97 → -3', hi: '100 से अंतर: 98 → -2, 97 → -3', proTip: { en: 'How far is each number from 100?', hi: 'प्रत्येक अंक 100 से कितना दूर है?' } },
        { en: 'Cross subtract: 98 - 3 = 95', hi: 'क्रॉस घटाव: 98 - 3 = 95', proTip: { en: 'Subtract the other number\'s gap (or 97 - 2 = 95).', hi: 'दूसरे अंक का अंतर घटाएँ (या 97 - 2 = 95)।' } },
        { en: 'Multiply gaps: (-2) × (-3) = 6', hi: 'अंतर गुणा: (-2) × (-3) = 6', proTip: { en: 'Negative × negative = positive. Write as 06.', hi: 'ऋण × ऋण = धनात्मक। 06 लिखें।' } },
        { en: 'Join parts: 95 and 06 → 9506', hi: 'भाग जोड़ें: 95 और 06 → 9506', proTip: { en: 'Put the cross result and product side by side.', hi: 'क्रॉस परिणाम और गुणनफल साथ रखें।' } },
      ],
      pros: ['speed', 'mental', 'exam'],
      timeEstimate: 12,
    },
    practice: [
      { q: '99 × 98', a: 9702, hint: { en: '-1 & -2, cross subtract, multiply, join', hi: '-1 और -2, क्रॉस घटाव, गुणा, जोड़ें' } },
      { q: '95 × 96', a: 9120, hint: { en: 'Deficiencies −5 & −4: cross subtract, then multiply', hi: 'कमी −5 और −4: क्रॉस घटाव, फिर गुणा' } },
      { q: '97 × 93', a: 9021, hint: { en: 'Deficiencies −3 & −7: cross subtract, then multiply', hi: 'कमी −3 और −7: क्रॉस घटाव, फिर गुणा' } },
      { q: '94 × 99', a: 9306, hint: { en: 'Deficiencies −6 & −1: cross subtract, then multiply', hi: 'कमी −6 और −1: क्रॉस घटाव, फिर गुणा' } },
      { q: '92 × 98', a: 9016, hint: { en: 'Deficiencies −8 & −2: cross subtract, then multiply', hi: 'कमी −8 और −2: क्रॉस घटाव, फिर गुणा' } },
    ],
    masteredText: { en: 'You can multiply big numbers near 100 easily!', hi: 'अब आप 100 के पास बड़े अंक आसानी से गुणा कर सकते हैं!' },
  },
  {
    id: 'divisibility-9',
    category: 'divide',
    difficulty: 1,
    icon: '9',
    en: 'Divisible by 9?',
    hi: '9 से भाज्य?',
    benefitChip: { en: 'Check in 1 sec', hi: '1 सेकंड में जाँच' },
    duration: '3 min',
    whenToUse: {
      en: 'Best for: checking answers, divisibility rules',
      hi: 'सबसे अच्छा: जवाब जाँचना, भाज्यता नियम',
    },
    whenNotToUse: {
      en: 'To find the actual quotient, you still need division.',
      hi: 'वास्तविक भागफल के लिए भाग करना होगा।',
    },
    whyVedic: [
      { type: 'verify', en: 'Verify any large number instantly', hi: 'किसी भी बड़ी संख्या की तुरंत जाँच' },
      { type: 'speed', en: 'Faster than doing the full division', hi: 'पूरा भाग करने से तेज़' },
      { type: 'exam', en: 'Catch mistakes before submitting', hi: 'जमा करने से पहले गलती पकड़ें' },
      { type: 'pattern', en: 'Digit sum rule works every time', hi: 'अंक जोड़ नियम हर बार काम करता है' },
    ],
    regular: {
      steps: [
        { en: 'Divide 4,536 ÷ 9 using long division', hi: '4,536 ÷ 9 लंबा भाग करें' },
        { en: '9 into 45 = 5, subtract…', hi: '45 में 9 = 5, घटाएँ…' },
        { en: 'Bring down digits, repeat', hi: 'अंक नीचे लाएँ, दोहराएँ' },
        { en: 'Takes 30+ seconds', hi: '30+ सेकंड लगते हैं' },
      ],
      timeEstimate: 28,
    },
    vedic: {
      steps: [
        { en: 'Add all digits: 4 + 5 + 3 + 6 = 18', hi: 'सभी अंक जोड़ें: 4 + 5 + 3 + 6 = 18', proTip: { en: 'Add every digit of the number.', hi: 'संख्या का हर अंक जोड़ें।' } },
        { en: 'Check digit sum: 18 ÷ 9 = 2 ✓', hi: 'अंक-योग जाँचें: 18 ÷ 9 = 2 ✓', proTip: { en: 'If the sum divides by 9, the number does too. (If sum > 9, add digits again: 1+8=9)', hi: 'योग 9 से भाज्य हो तो संख्या भी। (योग > 9 हो तो फिर जोड़ें: 1+8=9)' } },
        { en: 'Yes — 4,536 is divisible by 9', hi: 'हाँ — 4,536, 9 से भाज्य है', proTip: { en: 'Use this to verify answers without full division.', hi: 'पूरा भाग किए बिना जवाब जाँचने के लिए उपयोग करें।' } },
      ],
      pros: ['verify', 'speed', 'exam'],
      timeEstimate: 4,
    },
    practice: [
      { q: 'Is 2,871 divisible by 9?', a: 1, hint: { en: '2+8+7+1=18, yes!', hi: '2+8+7+1=18, हाँ!' }, isYesNo: true },
      { q: 'Is 5,436 divisible by 9?', a: 1, hint: { en: '5+4+3+6=18, yes!', hi: '5+4+3+6=18, हाँ!' }, isYesNo: true },
      { q: 'Is 1,234 divisible by 9?', a: 0, hint: { en: '1+2+3+4=10, no', hi: '1+2+3+4=10, नहीं' }, isYesNo: true },
      { q: 'Is 9,999 divisible by 9?', a: 1, hint: { en: '9+9+9+9=36, yes!', hi: '9+9+9+9=36, हाँ!' }, isYesNo: true },
      { q: 'Is 7,263 divisible by 9?', a: 1, hint: { en: '7+2+6+3=18, yes!', hi: '7+2+6+3=18, हाँ!' }, isYesNo: true },
    ],
    masteredText: { en: 'You can check divisibility by 9 in a flash!', hi: 'अब आप 9 से भाज्यता झटपट जाँच सकते हैं!' },
  },
  {
    id: 'complement-10',
    category: 'mental',
    difficulty: 1,
    icon: '10',
    en: 'Complements to 10',
    hi: '10 तक पूरक',
    benefitChip: { en: 'Lightning add', hi: 'बिजली जैसी जोड़' },
    duration: '3 min',
    whenToUse: {
      en: 'Best for: adding long columns, mental addition',
      hi: 'सबसे अच्छा: लंबे जोड़, मानसिक जोड़',
    },
    whenNotToUse: {
      en: 'Simple 2-number adds may not need this trick.',
      hi: 'साधारण 2-अंक जोड़ में यह तरीका जरूरी नहीं।',
    },
    whyVedic: [
      { type: 'speed', en: 'Add left-to-right, no carrying mess', hi: 'बाएँ से दाएँ जोड़ें, कैरी की झंझट नहीं' },
      { type: 'mental', en: 'Pairs that make 10 simplify everything', hi: '10 बनाने वाले जोड़े सब आसान करते हैं' },
      { type: 'confidence', en: 'Foundation for all Vedic maths', hi: 'सभी वैदिक गणित की नींव' },
      { type: 'pattern', en: '7+3, 6+4, 8+2 — always 10', hi: '7+3, 6+4, 8+2 — हमेशा 10' },
    ],
    regular: {
      steps: [
        { en: 'Add 47 + 38 right to left', hi: '47 + 38 दाएँ से बाएँ जोड़ें' },
        { en: '7+8=15, write 5 carry 1', hi: '7+8=15, 5 लिखें 1 कैरी' },
        { en: '4+3+1=8', hi: '4+3+1=8' },
        { en: 'Answer: 85', hi: 'जवाब: 85' },
      ],
      timeEstimate: 15,
    },
    vedic: {
      steps: [
        { en: 'Round 38 up to 40 (add 2)', hi: '38 को 40 पर पूर्णांक करें (+2)', proTip: { en: 'Find the next easy round number.', hi: 'अगली आसान पूर्ण संख्या खोजें।' } },
        { en: 'Balance: take 2 from 47 → 45', hi: 'संतुलन: 47 से 2 लें → 45', proTip: { en: 'Subtract the same amount from the other number.', hi: 'दूसरी संख्या से उतना ही घटाएँ।' } },
        { en: 'Add the round number: 45 + 40 = 85', hi: 'पूर्ण संख्या जोड़ें: 45 + 40 = 85', proTip: { en: 'Now the addition is easy!', hi: 'अब जोड़ आसान है!' } },
      ],
      pros: ['mental', 'speed', 'confidence'],
      timeEstimate: 6,
    },
    practice: [
      { q: '67 + 29', a: 96, hint: { en: 'Round 29 up to 30; subtract 1 from 67 first', hi: '29 को 30 बनाएँ; पहले 67 से 1 घटाएँ' } },
      { q: '48 + 37', a: 85, hint: { en: 'Round 37 up to 40; subtract 3 from 48 first', hi: '37 को 40 बनाएँ; पहले 48 से 3 घटाएँ' } },
      { q: '56 + 28', a: 84, hint: { en: 'Round 28 up to 30; subtract 2 from 56 first', hi: '28 को 30 बनाएँ; पहले 56 से 2 घटाएँ' } },
      { q: '39 + 46', a: 85, hint: { en: 'Round 39 up to 40; subtract 1 from 46 first', hi: '39 को 40 बनाएँ; पहले 46 से 1 घटाएँ' } },
      { q: '58 + 27', a: 85, hint: { en: 'Round 27 up to 30; subtract 3 from 58 first', hi: '27 को 30 बनाएँ; पहले 58 से 3 घटाएँ' } },
    ],
    masteredText: { en: 'You add numbers smarter using complements!', hi: 'अब आप पूरक से होशियारी से जोड़ते हैं!' },
  },
  {
    id: 'percent-10',
    category: 'percent',
    difficulty: 1,
    icon: '10%',
    en: 'Find 10% fast',
    hi: '10% जल्दी निकालें',
    benefitChip: { en: 'Move the dot', hi: 'बिंदु हिलाएँ' },
    duration: '3 min',
    whenToUse: {
      en: 'Best for: discounts, tips, marks, quick estimates',
      hi: 'सबसे अच्छा: छूट, टिप, अंक, जल्दी अनुमान',
    },
    whenNotToUse: {
      en: 'Weird percentages like 13% need the build-from-10% trick.',
      hi: '13% जैसे अजीब प्रतिशत के लिए 10% से बनाने की विधि लगेगी।',
    },
    whyVedic: [
      { type: 'speed', en: '10% = divide by 10 in one move', hi: '10% = एक ही कदम में 10 से भाग' },
      { type: 'mental', en: 'Just shift the decimal point left', hi: 'बस दशमलव बिंदु बाएँ खिसकाएँ' },
      { type: 'pattern', en: 'Foundation for 5%, 15%, 20%…', hi: '5%, 15%, 20%… की नींव' },
      { type: 'exam', en: 'Super common in word problems', hi: 'शब्दों वाले सवालों में बहुत आम' },
    ],
    regular: {
      steps: [
        { en: '10% of 240 — set up proportion', hi: '240 का 10% — अनुपात बनाएँ' },
        { en: '10/100 × 240', hi: '10/100 × 240' },
        { en: 'Simplify: 240 ÷ 10', hi: 'सरल करें: 240 ÷ 10' },
        { en: 'Answer: 24', hi: 'जवाब: 24' },
      ],
      timeEstimate: 20,
    },
    vedic: {
      steps: [
        { en: '10% means 1 out of every 10', hi: '10% का मतलब हर 10 में से 1', proTip: { en: '10% is always one-tenth of the number.', hi: '10% हमेशा संख्या का एक-दसवाँ होता है।' } },
        { en: 'Divide by 10: 240 ÷ 10 = 24', hi: '10 से भाग: 240 ÷ 10 = 24', proTip: { en: 'Or move the decimal one place left: 240. → 24.0', hi: 'या दशमलव एक स्थान बाएँ: 240. → 24.0' } },
        { en: 'Answer: 10% of 240 = 24', hi: 'जवाब: 240 का 10% = 24', proTip: { en: 'Use this as the base for 5%, 15%, 20% and more!', hi: 'इसे 5%, 15%, 20% और अधिक के आधार के रूप में उपयोग करें!' } },
      ],
      pros: ['speed', 'mental', 'pattern'],
      timeEstimate: 5,
    },
    practice: [
      { q: '10% of 80', a: 8, hint: { en: 'Move the decimal one place left (÷10)', hi: 'दशमलव एक अंक बाएँ (÷10)' } },
      { q: '10% of 350', a: 35, hint: { en: 'Move the decimal one place left (÷10)', hi: 'दशमलव एक अंक बाएँ (÷10)' } },
      { q: '10% of 120', a: 12, hint: { en: 'Move the decimal one place left (÷10)', hi: 'दशमलव एक अंक बाएँ (÷10)' } },
      { q: '10% of 500', a: 50, hint: { en: 'Move the decimal one place left (÷10)', hi: 'दशमलव एक अंक बाएँ (÷10)' } },
      { q: '10% of 90', a: 9, hint: { en: 'Move the decimal one place left (÷10)', hi: 'दशमलव एक अंक बाएँ (÷10)' } },
    ],
    masteredText: { en: 'You can find 10% of anything in a second!', hi: 'अब आप किसी भी संख्या का 10% एक सेकंड में निकाल सकते हैं!' },
  },
  {
    id: 'percent-25',
    category: 'percent',
    difficulty: 1,
    icon: '25%',
    en: 'Find 25% fast',
    hi: '25% जल्दी निकालें',
    benefitChip: { en: 'Just ÷ 4', hi: 'बस ÷ 4' },
    duration: '3 min',
    whenToUse: {
      en: 'Best for: quarter-off sales, 25% marks, splitting in 4',
      hi: 'सबसे अच्छा: चौथाई छूट, 25% अंक, 4 में बाँटना',
    },
    whenNotToUse: {
      en: 'For 33% or other fractions, use a different shortcut.',
      hi: '33% या अन्य भिन्न के लिए दूसरा शॉर्टकट उपयोग करें।',
    },
    whyVedic: [
      { type: 'speed', en: '25% = divide by 4 instantly', hi: '25% = तुरंत 4 से भाग' },
      { type: 'mental', en: '25% is the same as one quarter', hi: '25% = एक चौथाई के बराबर' },
      { type: 'pattern', en: 'Works for 25, 250, 2500…', hi: '25, 250, 2500… पर काम करता है' },
      { type: 'exam', en: 'Quarter fractions appear everywhere', hi: 'चौथाई भिन्न हर जगह आते हैं' },
    ],
    regular: {
      steps: [
        { en: '25% of 80 — set up proportion', hi: '80 का 25% — अनुपात बनाएँ' },
        { en: '25/100 × 80 = 1/4 × 80', hi: '25/100 × 80 = 1/4 × 80' },
        { en: '80 ÷ 4 = 20', hi: '80 ÷ 4 = 20' },
        { en: 'Answer: 20', hi: 'जवाब: 20' },
      ],
      timeEstimate: 18,
    },
    vedic: {
      steps: [
        { en: '25% = 1/4 (one quarter)', hi: '25% = 1/4 (एक चौथाई)', proTip: { en: 'Think “quarter” — 25¢ is a quarter of ₹1', hi: '“चौथाई” सोचें — 25 पैसे = ₹1 का चौथाई' } },
        { en: 'Divide by 4: 80 ÷ 4 = 20', hi: '4 से भाग: 80 ÷ 4 = 20', proTip: { en: 'Half of half also works: 80→40→20', hi: 'आधा-आधा भी चलेगा: 80→40→20' } },
        { en: 'Answer: 25% of 80 = 20', hi: 'जवाब: 80 का 25% = 20', proTip: { en: 'Same trick as ×25 — divide by 4 first!', hi: '×25 जैसा ही — पहले 4 से भाग!' } },
      ],
      pros: ['speed', 'mental', 'pattern'],
      timeEstimate: 6,
    },
    practice: [
      { q: '25% of 60', a: 15, hint: { en: '25% means divide by 4', hi: '25% = 4 से भाग' } },
      { q: '25% of 200', a: 50, hint: { en: '25% means divide by 4', hi: '25% = 4 से भाग' } },
      { q: '25% of 48', a: 12, hint: { en: '25% means divide by 4', hi: '25% = 4 से भाग' } },
      { q: '25% of 100', a: 25, hint: { en: '25% means divide by 4', hi: '25% = 4 से भाग' } },
      { q: '25% of 36', a: 9, hint: { en: '25% means divide by 4', hi: '25% = 4 से भाग' } },
    ],
    masteredText: { en: 'You can find 25% by dividing by 4!', hi: 'अब आप 25% चार से भाग करके निकाल सकते हैं!' },
  },
  {
    id: 'percent-build',
    category: 'percent',
    difficulty: 2,
    icon: '15%',
    en: 'Build % from 10%',
    hi: '10% से % बनाएँ',
    benefitChip: { en: 'Mix & match', hi: 'मिलाएँ और बनाएँ' },
    duration: '4 min',
    whenToUse: {
      en: 'Best for: 5%, 15%, 20%, 30% — any % built from 10%',
      hi: 'सबसे अच्छा: 5%, 15%, 20%, 30% — 10% से बनने वाले सभी %',
    },
    whenNotToUse: {
      en: 'For 10% or 25%, use the direct shortcuts instead.',
      hi: '10% या 25% के लिए सीधे शॉर्टकट उपयोग करें।',
    },
    whyVedic: [
      { type: 'pattern', en: '5% = half of 10%, 20% = double 10%', hi: '5% = 10% का आधा, 20% = 10% का दोगुना' },
      { type: 'mental', en: 'Break any % into easy pieces', hi: 'किसी भी % को आसान टुकड़ों में तोड़ें' },
      { type: 'speed', en: 'Faster than setting up fractions', hi: 'भिन्न बनाने से तेज़' },
      { type: 'exam', en: 'Handles 15%, 35%, 45% in exams', hi: 'परीक्षा में 15%, 35%, 45% हल करता है' },
    ],
    regular: {
      steps: [
        { en: '15% of 200 — set up 15/100 × 200', hi: '200 का 15% — 15/100 × 200 बनाएँ' },
        { en: 'Simplify: 15 × 200 ÷ 100', hi: 'सरल करें: 15 × 200 ÷ 100' },
        { en: 'Calculate: 3000 ÷ 100 = 30', hi: 'गणना: 3000 ÷ 100 = 30' },
        { en: 'Answer: 30', hi: 'जवाब: 30' },
      ],
      timeEstimate: 25,
    },
    vedic: {
      steps: [
        { en: 'Split 15% = 10% + 5%', hi: '15% = 10% + 5% में तोड़ें', proTip: { en: 'Break the % into 10% chunks you know!', hi: '% को ज्ञात 10% के टुकड़ों में तोड़ें!' } },
        { en: 'Find 10% of 200: 200 ÷ 10 = 20', hi: '200 का 10%: 200 ÷ 10 = 20', proTip: { en: 'Always find 10% first — it\'s the building block.', hi: 'हमेशा पहले 10% निकालें — यह नींव है।' } },
        { en: 'Find 5%: half of 20 = 10', hi: '5%: 20 का आधा = 10', proTip: { en: '5% is always half of 10%.', hi: '5% हमेशा 10% का आधा होता है।' } },
        { en: 'Add: 20 + 10 = 30', hi: 'जोड़ें: 20 + 10 = 30', proTip: { en: '20% would be 20+20=40. Easy!', hi: '20% होगा 20+20=40। आसान!' } },
      ],
      pros: ['pattern', 'mental', 'exam'],
      timeEstimate: 10,
    },
    practice: [
      { q: '15% of 200', a: 30, hint: { en: 'Find 10%, then add half of that for 5% more', hi: '10% निकालें, फिर उसका आधा 5% के लिए जोड़ें' } },
      { q: '20% of 150', a: 30, hint: { en: 'Find 10%, then double it for 20%', hi: '10% निकालें, 20% के लिए दोगुना करें' } },
      { q: '5% of 80', a: 4, hint: { en: 'Find 10%, then take half for 5%', hi: '10% निकालें, 5% के लिए आधा लें' } },
      { q: '30% of 100', a: 30, hint: { en: 'Find 10%, then triple it for 30%', hi: '10% निकालें, 30% के लिए तिगुना करें' } },
      { q: '15% of 60', a: 9, hint: { en: 'Find 10%, then add half of that for 15%', hi: '10% निकालें, 15% के लिए उसका आधा जोड़ें' } },
    ],
    masteredText: { en: 'You can build any % from 10% pieces!', hi: 'अब आप 10% के टुकड़ों से कोई भी % बना सकते हैं!' },
  },
  {
    id: 'anurupyena-hcf',
    category: 'numbers',
    difficulty: 2,
    icon: 'HCF',
    phase: 2,
    grades: '5–8',
    selinaTag: { en: 'Playing with Numbers', hi: 'संख्याओं से खेल' },
    sutra: { en: 'Anurupyena', hi: 'आनुरूप्येण', meaning: { en: 'Proportionately', hi: 'अनुपात से' } },
    en: 'HCF the fast way',
    hi: 'HCF तेज़ी से',
    benefitChip: { en: 'No long division', hi: 'लंबा भाग नहीं' },
    duration: '4 min',
    whenToUse: {
      en: 'Best for: HCF of two numbers in Selina “Playing with Numbers”',
      hi: 'सबसे अच्छा: दो संख्याओं का HCF — Selina Playing with Numbers',
    },
    whenNotToUse: {
      en: 'For three+ numbers, find HCF of first two, then with the third.',
      hi: 'तीन या अधिक संख्याओं पर पहले दो का HCF, फिर तीसरे से।',
    },
    whyVedic: [
      { type: 'speed', en: 'Euclid steps feel like proportion — keep reducing', hi: 'यूक्लिड कदम अनुपात जैसे — घटाते रहें' },
      { type: 'mental', en: 'Works with smaller remainders, not huge factors', hi: 'छोटे शेष से काम — बड़े गुणनखंड नहीं' },
      { type: 'exam', en: 'Selina Grades 6–8 favourite', hi: 'Selina कक्षा 6–8 पसंदीदा' },
      { type: 'pattern', en: 'Same method for any two integers', hi: 'किसी भी दो पूर्णांकों पर एक विधि' },
    ],
    regular: {
      steps: [
        { en: 'List factors of 48: 1,2,3,4,6,8,12,16,24,48', hi: '48 के गुणनखंड: 1,2,3,4,6,8,12,16,24,48' },
        { en: 'List factors of 18: 1,2,3,6,9,18', hi: '18 के गुणनखंड: 1,2,3,6,9,18' },
        { en: 'Common factors: 1, 2, 3, 6', hi: 'साझे गुणनखंड: 1, 2, 3, 6' },
        { en: 'Greatest = HCF 6', hi: 'सबसे बड़ा = HCF 6' },
      ],
      timeEstimate: 40,
    },
    vedic: {
      steps: [
        { en: 'Divide larger by smaller: 48 ÷ 18 = 2, rem 12', hi: 'बड़े को छोटे से भाग: 48 ÷ 18 = 2, शेष 12', proTip: { en: 'Anurupyena: keep numbers in proportion by remainders.', hi: 'आनुरूप्येण: शेष से अनुपात बनाए रखें।' } },
        { en: 'Now HCF(18, 12): 18 ÷ 12 = 1, rem 6', hi: 'अब HCF(18, 12): 18 ÷ 12 = 1, शेष 6', proTip: { en: 'Replace: larger becomes previous smaller.', hi: 'बड़ा = पिछला छोटा।' } },
        { en: 'HCF(12, 6): 12 ÷ 6 = 2, rem 0', hi: 'HCF(12, 6): 12 ÷ 6 = 2, शेष 0', proTip: { en: 'When remainder is 0, the last divisor is HCF.', hi: 'शेष 0 हो तो अंतिम भाजक ही HCF है।' } },
        { en: 'HCF = 6', hi: 'HCF = 6', proTip: { en: 'Check: 48=6×8, 18=6×3 ✓', hi: 'जाँच: 48=6×8, 18=6×3 ✓' } },
      ],
      pros: ['speed', 'mental', 'exam'],
      timeEstimate: 15,
    },
    practice: [
      { q: 'HCF of 48 and 18', a: 6, hint: { en: 'Divide larger by smaller; repeat with remainder', hi: 'बड़े से छोटे में भाग; शेष से दोहराएँ' }, validate: 'hcf48_18' },
      { q: 'HCF of 36 and 24', a: 12, hint: { en: '36÷24 rem 12; 24÷12 rem 0', hi: '36÷24 शेष 12; 24÷12 शेष 0' }, validate: 'hcf36_24' },
      { q: 'HCF of 60 and 45', a: 15, hint: { en: '60÷45 rem 15; 45÷15 rem 0', hi: '60÷45 शेष 15; 45÷15 शेष 0' }, validate: 'hcf60_45' },
      { q: 'HCF of 84 and 36', a: 12, hint: { en: '84÷36 rem 12; 36÷12 rem 0', hi: '84÷36 शेष 12; 36÷12 शेष 0' }, validate: 'hcf84_36' },
      { q: 'HCF of 100 and 35', a: 5, hint: { en: '100÷35 rem 30… then 5', hi: '100÷35 शेष 30… फिर 5' }, validate: 'hcf100_35' },
    ],
    masteredText: { en: 'You can find HCF with Anurupyena remainders!', hi: 'अब आप आनुरूप्येण से HCF निकाल सकते हैं!' },
  },
  {
    id: 'anurupyena-lcm',
    category: 'numbers',
    difficulty: 2,
    icon: 'LCM',
    phase: 2,
    grades: '5–8',
    selinaTag: { en: 'Playing with Numbers', hi: 'संख्याओं से खेल' },
    sutra: { en: 'Anurupyena', hi: 'आनुरूप्येण', meaning: { en: 'Proportionately', hi: 'अनुपात से' } },
    en: 'LCM from HCF',
    hi: 'HCF से LCM',
    benefitChip: { en: 'One formula', hi: 'एक सूत्र' },
    duration: '3 min',
    whenToUse: {
      en: 'Best for: LCM after you know HCF — Selina Grades 6–8',
      hi: 'सबसे अच्छा: HCF जानने के बाद LCM — Selina कक्षा 6–8',
    },
    whenNotToUse: {
      en: 'If HCF is wrong, LCM will be wrong — check HCF first.',
      hi: 'HCF गलत हो तो LCM भी गलत — पहले HCF जाँचें।',
    },
    whyVedic: [
      { type: 'speed', en: 'LCM = (a × b) ÷ HCF — one line', hi: 'LCM = (a × b) ÷ HCF — एक पंक्ति' },
      { type: 'pattern', en: 'Same proportion idea as HCF', hi: 'HCF जैसा ही अनुपात विचार' },
      { type: 'exam', en: 'Avoids long prime factor lists', hi: 'लंबी अभाज्य सूची से बचें' },
      { type: 'verify', en: 'Check: HCF × LCM = a × b', hi: 'जाँच: HCF × LCM = a × b' },
    ],
    regular: {
      steps: [
        { en: 'Prime factor 12 = 2² × 3', hi: '12 = 2² × 3 अभाज्य गुणनखंड' },
        { en: 'Prime factor 18 = 2 × 3²', hi: '18 = 2 × 3² अभाज्य गुणनखंड' },
        { en: 'Take highest powers: 2² × 3²', hi: 'उच्चतम घात: 2² × 3²' },
        { en: 'LCM = 4 × 9 = 36', hi: 'LCM = 4 × 9 = 36' },
      ],
      timeEstimate: 35,
    },
    vedic: {
      steps: [
        { en: 'Find HCF(12, 18) = 6', hi: 'HCF(12, 18) = 6 निकालें', proTip: { en: 'Use Anurupyena remainder method first.', hi: 'पहले आनुरूप्येण शेष विधि।' } },
        { en: 'LCM = (12 × 18) ÷ 6', hi: 'LCM = (12 × 18) ÷ 6', proTip: { en: 'Always product divided by HCF.', hi: 'हमेशा गुणनफल ÷ HCF।' } },
        { en: '216 ÷ 6 = 36', hi: '216 ÷ 6 = 36', proTip: { en: 'Verify: 6 × 36 = 216 = 12 × 18 ✓', hi: 'जाँच: 6 × 36 = 216 = 12 × 18 ✓' } },
      ],
      pros: ['speed', 'pattern', 'verify'],
      timeEstimate: 12,
    },
    practice: [
      { q: 'LCM of 12 and 18', a: 36, hint: { en: 'Find HCF first, then use (a×b)/HCF', hi: 'पहले HCF, फिर (a×b)/HCF' }, validate: 'lcm12_18' },
      { q: 'LCM of 8 and 12', a: 24, hint: { en: 'Find HCF first, then use (a×b)/HCF', hi: 'पहले HCF, फिर (a×b)/HCF' }, validate: 'lcm8_12' },
      { q: 'LCM of 15 and 20', a: 60, hint: { en: 'Find HCF first, then use (a×b)/HCF', hi: 'पहले HCF, फिर (a×b)/HCF' }, validate: 'lcm15_20' },
      { q: 'LCM of 9 and 6', a: 18, hint: { en: 'Find HCF first, then use (a×b)/HCF', hi: 'पहले HCF, फिर (a×b)/HCF' }, validate: 'lcm9_6' },
      { q: 'LCM of 14 and 21', a: 42, hint: { en: 'Find HCF first, then use (a×b)/HCF', hi: 'पहले HCF, फिर (a×b)/HCF' }, validate: 'lcm14_21' },
    ],
    masteredText: { en: 'You can get LCM from HCF in one formula!', hi: 'अब आप एक सूत्र से HCF से LCM निकाल सकते हैं!' },
  },
  {
    id: 'urdhva-fractions',
    category: 'fractions',
    difficulty: 2,
    icon: '½+',
    phase: 2,
    grades: '5–7',
    selinaTag: { en: 'Fractions & Rational Numbers', hi: 'भिन्न और परिमेय संख्याएँ' },
    sutra: { en: 'Urdhva-Tiryagbhyam', hi: 'ऊर्ध्वतिर्यग्भ्याम्', meaning: { en: 'Vertically and Crosswise', hi: 'ऊपर-नीचे और आड़ा' } },
    en: 'Add fractions (criss-cross)',
    hi: 'भिन्न जोड़ (क्रिस-क्रॉस)',
    benefitChip: { en: 'No LCM hunt', hi: 'LCM खोज नहीं' },
    duration: '4 min',
    whenToUse: {
      en: 'Best for: adding unlike fractions 5/7 + 3/4',
      hi: 'सबसे अच्छा: असमान भिन्न 5/7 + 3/4',
    },
    whenNotToUse: {
      en: 'If denominators are huge, simplify first — or use LCM when asked.',
      hi: 'हर बहुत बड़े हों तो पहले सरल करें — या LCM जब पूछा जाए।',
    },
    whyVedic: [
      { type: 'speed', en: 'Cross multiply instead of finding LCM', hi: 'LCM खोजने की जगह क्रिस-क्रॉस गुणा' },
      { type: 'steps', en: 'One formula: (ad + bc) / bd', hi: 'एक सूत्र: (ad + bc) / bd' },
      { type: 'exam', en: 'Selina Grades 5–7 fractions chapter', hi: 'Selina कक्षा 5–7 भिन्न अध्याय' },
      { type: 'mental', en: 'Works in your head for small numbers', hi: 'छोटी संख्याओं पर दिमाग में' },
    ],
    regular: {
      steps: [
        { en: 'Find LCM of 7 and 4 = 28', hi: '7 और 4 का LCM = 28' },
        { en: '5/7 = 20/28', hi: '5/7 = 20/28' },
        { en: '3/4 = 21/28', hi: '3/4 = 21/28' },
        { en: 'Add: 20/28 + 21/28 = 41/28', hi: 'जोड़: 20/28 + 21/28 = 41/28' },
      ],
      timeEstimate: 35,
    },
    vedic: {
      steps: [
        { en: 'Write 5/7 + 3/4', hi: 'लिखें 5/7 + 3/4', proTip: { en: 'Urdhva-Tiryag: cross the arms!', hi: 'ऊर्ध्वतिर्यग्: बाँहें क्रिस-क्रॉस!' } },
        { en: 'Numerator: 5×4 + 7×3 = 20 + 21 = 41', hi: 'अंश: 5×4 + 7×3 = 20 + 21 = 41', proTip: { en: 'Cross: first num × second den + second num × first den.', hi: 'क्रॉस: पहला अंश × दूसरा हर + दूसरा अंश × पहला हर।' } },
        { en: 'Denominator: 7 × 4 = 28', hi: 'हर: 7 × 4 = 28', proTip: { en: 'Just multiply the two denominators.', hi: 'बस दोनों हर गुणा करें।' } },
        { en: 'Answer: 41/28', hi: 'जवाब: 41/28', proTip: { en: 'Same as LCM method — fewer steps!', hi: 'LCM विधि जैसा — कम कदम!' } },
      ],
      pros: ['speed', 'steps', 'exam'],
      timeEstimate: 12,
    },
    practice: [
      { q: '5/7 + 3/4', a: { n: 41, d: 28 }, answerType: 'fraction', hint: { en: 'Cross multiply numerators; multiply denominators', hi: 'अंश क्रॉस गुणा; हरों का गुणा' }, validate: 'frac_5_7_3_4' },
      { q: '2/5 + 1/3', a: { n: 11, d: 15 }, answerType: 'fraction', hint: { en: 'Cross multiply numerators; multiply denominators', hi: 'अंश क्रॉस गुणा; हरों का गुणा' }, validate: 'frac_2_5_1_3' },
      { q: '1/4 + 2/3', a: { n: 11, d: 12 }, answerType: 'fraction', hint: { en: 'Cross multiply numerators; multiply denominators', hi: 'अंश क्रॉस गुणा; हरों का गुणा' }, validate: 'frac_1_4_2_3' },
      { q: '3/8 + 1/2', a: { n: 14, d: 16 }, answerType: 'fraction', hint: { en: 'Cross multiply numerators; multiply denominators', hi: 'अंश क्रॉस गुणा; हरों का गुणा' }, validate: 'frac_3_8_1_2' },
      { q: '4/9 + 1/6', a: { n: 33, d: 54 }, answerType: 'fraction', hint: { en: 'Cross multiply numerators; multiply denominators', hi: 'अंश क्रॉस गुणा; हरों का गुणा' }, validate: 'frac_4_9_1_6' },
    ],
    masteredText: { en: 'You can add unlike fractions with criss-cross!', hi: 'अब आप क्रिस-क्रॉस से असमान भिन्न जोड़ सकते हैं!' },
  },
  {
    id: 'digital-root-check',
    category: 'verify',
    difficulty: 1,
    icon: '९',
    phase: 2,
    grades: '5–8',
    selinaTag: { en: 'Number Operations — verify answers', hi: 'संख्या संक्रिया — जवाब जाँच' },
    sutra: { en: 'Digital Roots (Casting out 9s)', hi: 'अंक योग (9 निकालना)', meaning: { en: 'Check without full rework', hi: 'पूरा दोबारा किए बिना जाँच' } },
    en: 'Check answers with 9s',
    hi: '9 से जवाब जाँचें',
    benefitChip: { en: 'Instant verify', hi: 'तुरंत जाँच' },
    duration: '3 min',
    whenToUse: {
      en: 'Best for: double-checking multiply / SI / % answers',
      hi: 'सबसे अच्छा: गुणा / साधारण ब्याज / % जवाब दोबारा जाँचना',
    },
    whenNotToUse: {
      en: 'Digital roots catch many errors but not all — still careful on exams.',
      hi: 'अंक योग कई गलतियाँ पकड़ता है, सभी नहीं — परीक्षा में सावधान।',
    },
    whyVedic: [
      { type: 'verify', en: 'Know if 48×17=816 is possible in 2 seconds', hi: '2 सेकंड में जानें 48×17=816 संभव है या नहीं' },
      { type: 'speed', en: 'No need to redo the whole sum', hi: 'पूरा योग दोबारा करने की जरूरत नहीं' },
      { type: 'exam', en: 'Great before submitting Selina homework', hi: 'Selina होमवर्क जमा करने से पहले' },
      { type: 'pattern', en: 'Digit sum until one digit — same as ÷9 check', hi: 'एक अंक तक योग — ÷9 जाँच जैसा' },
    ],
    regular: {
      steps: [
        { en: 'To check 48 × 17, redo long multiplication', hi: '48 × 17 जाँचने के लिए लंबी गुणा दोबारा करें' },
        { en: '48 × 7 = 336', hi: '48 × 7 = 336' },
        { en: '48 × 10 = 480', hi: '48 × 10 = 480' },
        { en: 'Add: 336 + 480 = 816 ✓', hi: 'जोड़: 336 + 480 = 816 ✓' },
      ],
      timeEstimate: 25,
    },
    vedic: {
      steps: [
        { en: 'Digit root of 48: 4+8=12 → 1+2=3', hi: '48 का अंक योग: 4+8=12 → 1+2=3', proTip: { en: 'Keep adding digits until one digit left.', hi: 'एक अंक रह जाए तब तक जोड़ते रहें।' } },
        { en: 'Digit root of 17: 1+7=8', hi: '17 का अंक योग: 1+7=8', proTip: { en: 'Do the same for the other factor.', hi: 'दूसरे गुणनखंड के लिए भी।' } },
        { en: '3 × 8 = 24 → 2+4=6', hi: '3 × 8 = 24 → 2+4=6', proTip: { en: 'Product of roots → reduce again.', hi: 'मूलों का गुणनफल → फिर घटाएँ।' } },
        { en: '816: 8+1+6=15 → 1+5=6 ✓ matches!', hi: '816: 8+1+6=15 → 1+5=6 ✓ मेल!', proTip: { en: 'If roots match, answer is likely correct — without redoing the full multiply.', hi: 'मूल मिलें तो जवाब सही होने की संभावना — पूरी गुणा दोबारा किए बिना।' } },
      ],
      pros: ['verify', 'speed', 'exam'],
      timeEstimate: 6,
    },
    practice: [
      { q: 'Digit root of 48?', a: 3, hint: { en: 'Add all digits; repeat until one digit', hi: 'सभी अंक जोड़ें; एक अंक तक दोहराएँ' }, validate: 'dr48' },
      { q: 'Digit root of 99?', a: 9, hint: { en: 'Add all digits; repeat until one digit', hi: 'सभी अंक जोड़ें; एक अंक तक दोहराएँ' }, validate: 'dr99' },
      { q: 'Does 48×17=816 check OK?', a: 1, isYesNo: true, hint: { en: 'roots 3×8→6, 816→6 ✓', hi: 'मूल 3×8→6, 816→6 ✓' }, validate: 'verify48_17_816' },
      { q: 'Does 25×16=390 check OK?', a: 0, isYesNo: true, hint: { en: '2+5=7; 1+6=7; 7×7=49→13→4; 390→12→3 ✗', hi: 'मूल मेल नहीं' }, validate: 'verify25_16_390' },
      { q: 'Digit root of 123?', a: 6, hint: { en: 'Add all digits; repeat until one digit', hi: 'सभी अंक जोड़ें; एक अंक तक दोहराएँ' }, validate: 'dr123' },
    ],
    masteredText: { en: 'You can check answers with casting out 9s!', hi: 'अब आप 9 निकालकर जवाब जाँच सकते हैं!' },
  },
  {
    id: 'simple-interest',
    category: 'commercial',
    difficulty: 2,
    icon: 'SI',
    phase: 2,
    grades: '7–8',
    selinaTag: { en: 'Simple Interest & Percentages', hi: 'साधारण ब्याज और प्रतिशत' },
    sutra: { en: 'Percent build + Digital Roots', hi: 'प्रतिशत बनाएँ + अंक योग', meaning: { en: 'Fast SI with % tricks', hi: '% तरकीब से तेज़ SI' } },
    en: 'Simple Interest fast',
    hi: 'साधारण ब्याज तेज़',
    benefitChip: { en: 'P×R×T/100', hi: 'P×R×T/100' },
    duration: '4 min',
    whenToUse: {
      en: 'Best for: Selina SI — Principal × Rate × Time / 100',
      hi: 'सबसे अच्छा: Selina SI — मूल × दर × समय / 100',
    },
    whenNotToUse: {
      en: 'Compound interest needs a different formula.',
      hi: 'चक्रवृद्धि ब्याज के लिए अलग सूत्र।',
    },
    whyVedic: [
      { type: 'speed', en: 'Use 10% / 5% building blocks on P×R', hi: 'P×R पर 10% / 5% ब्लॉक' },
      { type: 'exam', en: 'Core Selina Grades 7–8 commercial maths', hi: 'Selina कक्षा 7–8 वाणिज्यिक गणित' },
      { type: 'verify', en: 'Check SI with digital root if numbers big', hi: 'बड़े अंक हों तो अंक योग से जाँच' },
      { type: 'mental', en: 'Often finish without long multiplication', hi: 'अक्सर लंबी गुणा बिना' },
    ],
    regular: {
      steps: [
        { en: 'SI = (P × R × T) / 100', hi: 'SI = (P × R × T) / 100' },
        { en: 'P=800, R=5, T=2 → 800 × 5 × 2', hi: 'P=800, R=5, T=2 → 800 × 5 × 2' },
        { en: '800 × 5 × 2 = 8000', hi: '800 × 5 × 2 = 8000' },
        { en: '8000 ÷ 100 = 80', hi: '8000 ÷ 100 = 80' },
      ],
      timeEstimate: 30,
    },
    vedic: {
      steps: [
        { en: 'Example: P=800, R=5%, T=2 years', hi: 'उदाहरण: P=800, R=5%, T=2 वर्ष', proTip: { en: 'Write SI = P × R × T ÷ 100', hi: 'लिखें SI = P × R × T ÷ 100' } },
        { en: 'Find 5% of 800 using 10%: 10%=80, half=40', hi: '800 का 5%: 10%=80, आधा=40', proTip: { en: 'Reuse your % Vedic tricks!', hi: 'अपनी % वैदिक तरकीबें दोबारा!' } },
        { en: 'For 2 years: 40 × 2 = 80', hi: '2 वर्ष: 40 × 2 = 80', proTip: { en: 'Multiply by time after rate of P.', hi: 'दर के बाद समय से गुणा।' } },
        { en: 'SI = 80', hi: 'SI = 80', proTip: { en: 'Check: (800×5×2)/100 = 80 ✓', hi: 'जाँच: (800×5×2)/100 = 80 ✓' } },
      ],
      pros: ['speed', 'exam', 'mental'],
      timeEstimate: 12,
    },
    practice: [
      { q: 'SI: P=800, R=5%, T=2', a: 80, hint: { en: 'Use SI = P×R×T ÷ 100', hi: 'SI = P×R×T ÷ 100 लगाएँ' }, validate: 'si800_5_2' },
      { q: 'SI: P=1000, R=10%, T=3', a: 300, hint: { en: 'Use SI = P×R×T ÷ 100', hi: 'SI = P×R×T ÷ 100 लगाएँ' }, validate: 'si1000_10_3' },
      { q: 'SI: P=500, R=8%, T=2', a: 80, hint: { en: 'Use SI = P×R×T ÷ 100', hi: 'SI = P×R×T ÷ 100 लगाएँ' }, validate: 'si500_8_2' },
      { q: 'SI: P=1200, R=5%, T=4', a: 240, hint: { en: 'Use SI = P×R×T ÷ 100', hi: 'SI = P×R×T ÷ 100 लगाएँ' }, validate: 'si1200_5_4' },
      { q: 'SI: P=600, R=7%, T=1', a: 42, hint: { en: 'Use SI = P×R×T ÷ 100', hi: 'SI = P×R×T ÷ 100 लगाएँ' }, validate: 'si600_7_1' },
    ],
    masteredText: { en: 'You can crack Simple Interest with % shortcuts!', hi: 'अब आप % शॉर्टकट से साधारण ब्याज हल कर सकते हैं!' },
  },
  {
    id: 'profit-loss',
    category: 'commercial',
    difficulty: 2,
    icon: 'P&L',
    phase: 2,
    grades: '7–8',
    selinaTag: { en: 'Profit, Loss & Percentages', hi: 'लाभ, हानि और प्रतिशत' },
    sutra: { en: 'Percent of CP', hi: 'क्रय मूल्य का %', meaning: { en: 'Profit% = profit/CP × 100', hi: 'लाभ% = लाभ/क्रय × 100' } },
    en: 'Profit & Loss %',
    hi: 'लाभ और हानि %',
    benefitChip: { en: 'Money sense', hi: 'पैसे की समझ' },
    duration: '4 min',
    whenToUse: {
      en: 'Best for: CP, SP, profit% and loss% word problems',
      hi: 'सबसे अच्छा: क्रय, विक्रय, लाभ% और हानि% शब्द समस्याएँ',
    },
    whenNotToUse: {
      en: 'Discount on marked price is a related but different idea.',
      hi: 'अंकित मूल्य पर छूट संबंधित पर अलग विचार है।',
    },
    whyVedic: [
      { type: 'exam', en: 'Selina commercial maths staple', hi: 'Selina वाणिज्यिक गणित का मुख्य भाग' },
      { type: 'speed', en: 'Combine with 10%/5% mental %', hi: '10%/5% मानसिक % के साथ' },
      { type: 'pattern', en: 'Always compare to Cost Price', hi: 'हमेशा क्रय मूल्य से तुलना' },
      { type: 'confidence', en: 'Real-life shopping maths', hi: 'असली खरीदारी गणित' },
    ],
    regular: {
      steps: [
        { en: 'CP=200, SP=240 → Profit = 240 − 200 = 40', hi: 'क्रय=200, विक्रय=240 → लाभ = 40' },
        { en: 'Profit% = (Profit ÷ CP) × 100', hi: 'लाभ% = (लाभ ÷ क्रय) × 100' },
        { en: '(40 ÷ 200) × 100', hi: '(40 ÷ 200) × 100' },
        { en: '0.2 × 100 = 20%', hi: '0.2 × 100 = 20%' },
      ],
      timeEstimate: 28,
    },
    vedic: {
      steps: [
        { en: 'CP = 200, SP = 240 → Profit = 40', hi: 'क्रय=200, विक्रय=240 → लाभ=40', proTip: { en: 'First find the money difference.', hi: 'पहले रुपये का अंतर निकालें।' } },
        { en: 'Profit% = 40/200 × 100', hi: 'लाभ% = 40/200 × 100', proTip: { en: 'Always divide by Cost Price.', hi: 'हमेशा क्रय मूल्य से भाग।' } },
        { en: '40/200 = 1/5 = 20%', hi: '40/200 = 1/5 = 20%', proTip: { en: 'Or: 10% of 200=20, so 40 is 20%.', hi: 'या: 200 का 10%=20, अतः 40 = 20%।' } },
      ],
      pros: ['exam', 'speed', 'confidence'],
      timeEstimate: 12,
    },
    practice: [
      { q: 'CP=200 SP=240. Profit%?', a: 20, hint: { en: 'Profit% = (SP−CP) ÷ CP × 100', hi: 'लाभ% = (SP−CP) ÷ CP × 100' }, validate: 'pl_profit20' },
      { q: 'CP=150 SP=180. Profit%?', a: 20, hint: { en: 'Profit% = (SP−CP) ÷ CP × 100', hi: 'लाभ% = (SP−CP) ÷ CP × 100' }, validate: 'pl_profit20b' },
      { q: 'CP=100 SP=80. Loss%?', a: 20, hint: { en: 'Loss% = (CP−SP) ÷ CP × 100', hi: 'हानि% = (CP−SP) ÷ CP × 100' }, validate: 'pl_loss20' },
      { q: 'CP=250 SP=300. Profit%?', a: 20, hint: { en: 'Profit% = (SP−CP) ÷ CP × 100', hi: 'लाभ% = (SP−CP) ÷ CP × 100' }, validate: 'pl_profit20c' },
      { q: 'CP=400 SP=360. Loss%?', a: 10, hint: { en: 'Loss% = (CP−SP) ÷ CP × 100', hi: 'हानि% = (CP−SP) ÷ CP × 100' }, validate: 'pl_loss10' },
    ],
    masteredText: { en: 'You can solve Profit & Loss % like a pro!', hi: 'अब आप लाभ-हानि % होशियारी से हल कर सकते हैं!' },
  },
  ...PHASE34_TECHNIQUES,
];

export const DAILY_CHALLENGE = {
  pro: { en: 'Today: cubes, algebra & trig', hi: 'आज: घन, बीजगणित और त्रिकोणमिति' },
  techniques: ['cube-near-10', 'urdhva-binomial', 'trig-identity'],
};

export function getTechnique(id) {
  return TECHNIQUES.find((t) => t.id === id);
}

export function getTechniquesByCategory(categoryId) {
  return TECHNIQUES.filter((t) => t.category === categoryId);
}
